...
- 已新增 `visualize_data` 数据可视化工具：模型可要求把数据画成折线图/柱状图/饼图（按年度、顾客公司、项目或源公司分组，数量或金额指标），matplotlib 生成 PNG 后通过 `/api/charts/<file>` 提供，前端直接渲染图片；Qwen 未自动调用图表工具时由本地规则 Agent 兜底生成。
